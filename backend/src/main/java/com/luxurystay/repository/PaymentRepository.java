...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.status = 'COMPLETED' AND p.booking.hotel.id = :hotelId")
    BigDecimal getTotalPaymentsByHotelId(@Param("hotelId") Long hotelId);

    @Query("SELECT COALESCE(SUM(p.amount), 0) FROM Payment p WHERE p.status = 'COMPLETED' AND p.createdAt BETWEEN :start AND :end")
    BigDecimal getPaymentsBetween(@Param("start") java.time.LocalDateTime start, @Param("end") java.time.LocalDateTime end);
